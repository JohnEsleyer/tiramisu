...
export class AudioAnalyzer {
    public async analyze(
    ...
    ): Promise<{ rms: number; bands: number[] }[]> {
        let AudioVisualizer;
        try {
            // Import the generated glue code directly from source
            // @ts-ignore
            ...
            console.warn("Falling back to silent mode.");
        ...
        console.log(
            `   🎵 Analyzing Audio (DC Filtered + Web Audio Physics): ${audioFile}`,
        );

        const absolutePath = isAbsolute(audioFile)
            ? audioFile
            : join(process.cwd(), audioFile);
        ...
        for (let f = 0; f < totalFrames; f++) {
            ...
            const rms = Math.min(
                Math.sqrt(sumSq / (frameSamples.length || 1)) * 2.0,
                1.0,
            );
